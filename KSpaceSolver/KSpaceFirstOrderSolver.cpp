...
void KSpaceFirstOrderSolver::sumPressureTermsLinearLossless()
{
  ...
  #pragma omp parallel for simd schedule(static) aligned(rhoX, rhoY, rhoZ, c2Matrix, p : kDataAlignment)
  for (size_t i = 0; i < nElements; i++)
  {
    const float c2      = (c0ScalarFlag) ?  c2Scalar : c2Matrix[i];
                                                                ...
                                                                RealMatrix& velocityGradientSum)
{
  ...
  #pragma omp parallel for simd schedule(static) aligned(rhoX, rhoY, rhoZ,pDensitySum : kDataAlignment)
...
void KSpaceFirstOrderSolver::computeInitialVelocityUniform()
{
  ...
  if (simulationDimension == SD::k3D)
  {
    ...
    #pragma omp parallel for simd schedule(static) aligned(dtRho0SgzMatrix, dpdzSgz, uzSgz : kDataAlignment)
    for (size_t i = 0; i < nElements; i++)
    {
...
void KSpaceFirstOrderSolver::generateC2()
{
  ...
  { // Matrix values
    ...
    #pragma omp parallel for simd schedule(static) aligned(c2 : kDataAlignment)
    for (size_t i = 0; i < nElements; i++)
    {
      c2[i] = c2[i] * c2[i];
    }
